...
class Viewer2D:
    ...
    def __init__(
        ...
        if gene_marker_file is None:
            gene_marker_file = importlib_resources.files("coppafisher.plot.results_viewer").joinpath("gene_colour.csv")
        ...
        self.probs_gene_no = np.argmax(nb.call_spots.gene_probabilities, axis=1)
        self.probs_score = np.max(nb.call_spots.gene_probabilities, axis=1)
