...
def generate_global_image(
    ...
    Args:
        ...
        tiles_given (list of int): tiles_given[i] is the tile index for images[i].
    ...
    if np.issubdtype(output_dtype, np.floating):
        ...
        output[:] = np.iinfo(output_dtype).max
    ...
    for t_i, t in enumerate(tqdm.tqdm(nbp_basic.use_tiles, desc="Fusing images", unit="tile", disable=silent)):
        ...
        t_image = images.pop(t_index)
        if output_dtype not in (np.float16, np.float32, np.float64, np.float128):
            t_image = t_image.round()
