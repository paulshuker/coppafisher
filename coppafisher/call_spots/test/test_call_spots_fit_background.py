...
from coppafisher.call_spots import background
...
def test_fit_background() -> None:
    ...
    residual1, coef1, background_vectors1 = background.fit_background(torch.asarray(spot_colours), weight_shift=0)
    residual1 = residual1.numpy()
    coef1 = coef1.numpy()
    background_vectors1 = background_vectors1.numpy()
    assert (
        residual1.shape == spot_colours.shape
    ...
    assert background_vectors1.shape == (
        n_channels,
        n_rounds,
        n_channels,
    ...
    coef1 = coef1.numpy()
    background_vectors1 = background_vectors1.numpy()
    residual2, coef2, background_vectors2 = background.fit_background(torch.asarray(spot_colours), weight_shift=10)
    residual2 = residual2.numpy()
    coef2 = coef2.numpy()
    background_vectors2 = background_vectors2.numpy()
