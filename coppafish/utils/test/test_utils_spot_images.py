...
def test_get_spot_images():
    ...
    assert np.all(
        np.isnan(spot_images.get_spot_images(np.zeros((5, 3)), np.array([[20, 10], [-10, -100]]), shape=[3, 5]))
    ...
    assert np.all(
        np.isnan(
            spot_images.get_spot_images(np.zeros((5, 3)), np.array([[20, 10], [-10, -100]]), shape=np.array([3, 5]))
        )
    ), "Expected only nan values when out of bounds 2d spot given"
    assert np.all(
        np.isnan(
...
def test_get_average_spot_image():
    ...
    assert np.allclose(
        spot_images.get_average_spot_image(spot_images_2d, "mean"), np.mean(fill_values[2:])
    ), "Unexpected 2d output with nan values"
    assert np.allclose(
        spot_images.get_average_spot_image(spot_images_3d, "mean"), np.mean(fill_values[2:])
    ), "Unexpected 3d output with nan values"
    assert np.allclose(
        spot_images.get_average_spot_image(spot_images_2d, "median"), np.median(fill_values[2:])
    ), "Unexpected 2d output with nan values"
    assert np.allclose(
        spot_images.get_average_spot_image(spot_images_3d, "median"), np.median(fill_values[2:])
    ), "Unexpected 3d output with nan values"
