...
def get_spot_images(image: np.ndarray, spot_yxz: np.ndarray, shape: Union[np.ndarray, List[int]]) -> np.ndarray:
    ...
    if min(np.array(shape) % 2) == 0:
        log.error(ValueError(f"Require shape to be odd in each dimension but given shape was {shape}."))
    mid_index = (
    ...
    with tqdm(total=n_spots) as pbar:
        pbar.set_description("Loading in spot images from image")
        for s in range(n_spots):
            ...
            if len(shape) == 2:
                ...
                spot_images[
                    s,
                    spot_images_min_index[0] : spot_images_max_index[0],
                ...
                spot_images[
                    ...
                    spot_images_min_index[0] : spot_images_max_index[0],
                    spot_images_min_index[1] : spot_images_max_index[1],
                    spot_images_min_index[2] : spot_images_max_index[2],
                ] = small_im
            pbar.update(1)
    pbar.close()
...
def get_average_spot_image(
    spot_images: np.ndarray, av_type: str = "mean", symmetry: Optional[str] = None, annulus_width: float = 1.0
) -> np.ndarray:
    """
    ...
    if av_type == "mean":
        ...
        log.error(ValueError(f"av_type must be 'mean' or 'median' but value given was {av_type}"))
    ...
    if symmetry is None:
        ...
        X, Y = np.meshgrid(
            np.arange(spot_images.shape[1]) - mid_index[0], np.arange(spot_images.shape[2]) - mid_index[1]
        )
        d = np.sqrt(X**2 + Y**2)
        ...
        if symmetry == "annulus_3d":
            if spot_images.ndim != 4:
                log.error(ValueError("Must give 3D images with symmetry = 'annulus_3d'"))
            n_z = spot_images.shape[3]
            if n_z % 2 == 0:
                log.error(ValueError("Must have odd number of z-planes with symmetry = 'annulus_3d'"))
    ...
    if symmetry is not None:
        ...
        if not is_odd.all():
            log.warn(
                f"spot_images shape is {av_image.shape} which is even in some dimensions."
